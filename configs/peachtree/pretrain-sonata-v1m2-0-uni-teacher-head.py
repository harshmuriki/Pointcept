...
model = dict(
    ...
    backbone=dict(
        type="PT-v3m2",
        in_channels=9,  # coord (3) + color (3) + normal (3) for peachtree
...
# scheduler settings
epoch = 100  # Reduced epochs for smaller dataset
base_lr = 0.002  # Reduced learning rate for stability
...
# dataset settings
transform = [
    dict(type="GridSample", grid_size=0.005, hash_type="fnv", mode="train"),  # Smaller grid size for tree detail
    ...
    dict(
        ...
        global_transform=[
            ...
            dict(type="RandomScale", scale=[0.95, 1.05]),  # Smaller scale variation for trees
            dict(type="RandomRotate", angle=[-1, 1], axis="z", center=[0, 0, 0], p=0.8),
            dict(type="RandomRotate", angle=[-1 / 32, 1 / 32], axis="x", p=0.8),  # Smaller rotation for trees
            dict(type="RandomRotate", angle=[-1 / 32, 1 / 32], axis="y", p=0.8),
        ...
        max_size=50000,  # Smaller max size for tree point clouds
    ),
    dict(type="ToTensor"),
    dict(type="Update", keys_dict={"grid_size": 0.005}),  # Updated to match new grid size
    dict(
        type="Collect",
        keys=(
            "global_origin_coord",
